...
## HISTORY REWRITE 2026-09-24: author identity normalization

Owner decision: this is a personal account, so public commit metadata carries
only the account's noreply identity. One 2026-07-07 commit here carried a
work email; `git filter-repo --mailmap` remapped it, which gave every later
commit a new hash. Only commit metadata changed: every rewritten commit was
verified to carry a tree byte-identical to its original, and HEAD's tree is
unchanged. Two commit messages that named a protected private repository were
genericized in the same pass, so the pre-push hook passes on its own terms
with no bypass.

- Hashes cited in this file and in PROGRESS.md were remapped to the new
  history. `v1.1.0` was retagged onto its rewritten commit. Consumer
  provenance records stamped before this date name pre-rewrite hashes; they
  are history, and the restamp round that followed the rewrite brought every
  consumer current.
- RULE: a history rewrite on a fleet repo is a full restamp round. It lifts
  the default-branch ruleset for the force push only, re-applies it in the
  same script, and re-reads it (`github-rulesets.sh --check`) before the
  round is called done.
- RULE: GitHub keeps pre-rewrite commits reachable by SHA, and pull request
  refs keep them reachable indefinitely. A rewrite hides metadata from clones
  and from the branch view. It does not erase it; only GitHub Support can
  purge cached objects.
- RULE: set the identity before the first commit. Global `user.email` is the
  noreply address, and any per-repo override must match it.
...
- RESTORED (2026-08-19): the five detailed lines above were added by
  5538be9 and 4c833e7, then accidentally deleted hours later by c8ad8a3
...
- RESOLVED DRIFT (2026-08-19): the repo was renamed uebersicht-claude-tokens
  to claude-tokens on 2026-08-18 (profile repo 09cfacd, widget repo b2b9126).
...
- Python is pinned exactly in `.python-version`; direct dependencies live in
  ...
  both pins moved together in canonical c638cb5. That made the cost model
